...
class ParticleFiler():
    ...
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber(rospy.get_param("~odometry_topic", "odom"), Odometry, self.odomCB,
                                         queue_size=1)
    ...
    def publish_tf(self, pose, stamp=None):
        ...
        # Original transform components
        translation = (pose[0], pose[1], 0.5)
        rotation = tf.transformations.quaternion_from_euler(0, 0, pose[2])

        # Calculate the inverse rotation
        inverse_rotation = tf.transformations.quaternion_inverse(rotation)

        # Calculate the inverse translation
        inverse_translation = tf.transformations.quaternion_multiply(
            tf.transformations.quaternion_multiply(inverse_rotation, (-pose[0], -pose[1], -0.125, 0)),
            rotation)[:3]

        # Publish the inverse transform from 'map' to 'robot_name/laser'
        self.pub_tf.sendTransform(inverse_translation, inverse_rotation, stamp, "map", f"{self.ROBOT_NAME}/laser")
        ...
        #                           stamp, f"{self.ROBOT_NAME}/laser", "map")
        ...
        #
        # # Publish transform
        # self.pub_tf.sendTransform(map_laser_pos, map_laser_rotation, stamp, f"{self.ROBOT_NAME}/base_link", "map")
    ...
    def publish_scan(self, angles, ranges):
        ...
        ls = LaserScan()
        ls.header = Utils.make_header(f"{self.ROBOT_NAME}/laser", stamp=self.last_stamp)
    ...
    def initialize_particles_pose(self, pose):
        ...
        self.particles[:, 2] = Utils.quaternion_to_angle(pose.orientation) + np.random.normal(loc=0.0, scale=0.4,
    ...
    def sensor_model(self, proposal_dist, obs, weights):
        ...
        if self.RANGELIB_VAR == VAR_RADIAL_CDDT_OPTIMIZATIONS:
            if "cddt" in self.WHICH_RM:
                ...
                self.range_method.calc_range_many_radial_optimized(num_rays, self.downsampled_angles[0],
                                                                   self.downsampled_angles[-1], self.queries,
                                                                   self.ranges)
            ...
            self.queries[:, :] = proposal_dist[:, :]
            self.range_method.calc_range_repeat_angles_eval_sensor_model(self.queries, self.downsampled_angles, obs,
                                                                         self.weights)
            ...
            if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
                print("sensor_model: init: ", np.round((t_init - t_start) / t_total, 2), "range:",
                      np.round((t_range - t_init) / t_total, 2), \
                      "eval:", np.round((t_eval - t_range) / t_total, 2), "squash:",
    ...
    def MCL(self, a, o):
        ...
        if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
            print("MCL: propose: ", np.round((t_propose - t) / t_total, 2), "motion:",
                  np.round((t_motion - t_propose) / t_total, 2), \
                  "sensor:", np.round((t_sensor - t_motion) / t_total, 2), "norm:",
